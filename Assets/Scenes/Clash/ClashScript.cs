...
public class ClashScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player1 = new ClashPlayer("51,38,46,41,37,52,42,49,45,53,50,48,54,43,39,47,40,44");
        player2 = new ClashPlayer("49,37,42,47,38,53,45,40,46,52,48,51,39,50,43,41,54,44");
        // set animationQueue
        // clashAnimationQueue = new ClashAnimationQueue("");
        clashAnimationQueue = new ClashAnimationQueue("p,dws,1,0*p,p,1,0*s,dws,1,0*s,p,1,0*p,dws,1,0*p,p,2,0*s,dws,1,0*s,p,2,0*p,dws,1,0*p,p,3,0*s,dws,1,0*s,p,3,0*p,dws,1,0*p,p,4,0*s,dws,1,0*s,p,4,0*p,dws,1,0*p,p,5,0*s,dws,1,0*s,p,5,0*p,dws,1,0*p,p,6,0*s,dws,1,0*s,p,6,0*b,r,3,6*s,dws,1,0*s,p,6,0*b,gd,1,1*b,r,5,5*p,dws,1,0*p,p,5,0*s,dws,1,0*s,p,5,0*b,gd,0,1*b,r,3,6*p,dws,1,0*p,p,4,0*s,dws,1,0*s,p,6,0*s,pow,2,1*s,pow,2,6*b,gd,0,1*b,r,4,3*p,dws,1,0*p,p,4,0*s,pow,2,1*s,pow,2,6*b,gd,0,1*b,r,6,4*s,dws,1,0*s,p,4,0*p,dws,1,0*p,p,6,0*p,pow,4,2*b,gd,1,0*b,r,6,1*s,dws,1,0*s,p,1,0*p,dws,1,0*p,p,6,0*p,pow,4,2*b,gd,1,0*b,r,1,1*p,dws,1,0*p,p,1,0*p,pow,4,2*p,pow,2,1*p,pow,2,2*p,pow,2,4*p,pow,2,5*p,des,0,5*p,pow,2,6*s,pow,2,2*s,pow,2,6*b,gd,1,1*b,r,6,6*s,dws,1,0*s,p,6,0*p,pow,3,1*b,gd,1,0*b,r,2,3*p,dws,1,0*p,p,2,0*s,pow,17,5*b,gd,0,1*b,r,3,4*s,dws,1,0*s,p,5,0*p,pow,3,1*p,pow,1,1*p,pow,1,2*s,des,0,5*b,gd,1,0*b,r,2,3*p,dws,1,0*p,p,2,0*b,gd,0,1*b,r,6,4*s,dws,1,0*s,p,2,0*b,gd,1,0*b,r,4,1*s,dws,1,0*s,p,2,0*p,dws,1,0*p,p,2,0*p,pow,1,2*b,gd,1,0*b,r,1,6*p,dws,1,0*p,p,2,0*s,dws,1,0*s,p,2,0*s,pow,1,2*b,gd,0,1*b,r,3,2*b,gd,1,1*b,ft,0,0");
        //
    ...
    void Update()
    {
        switch (clashAnimationQueue.getCurrentAnimationType())
        {
            ...
            case ClashAnimationType.DestroyCard:
                ...
                break;
            case ClashAnimationType.FighterToBottomOfDeck:
                FighterToBottomOfDeckAnimation();
                break;
            case ClashAnimationType.PlayFromDiscard:
                PlayFromDiscardAnimation();
                break;
            default:
                NextAnimation();
                break;
    ...
    void FighterToBottomOfDeckAnimation()
    {
        int cardSlot = clashAnimationQueue.getCurrentAnimation().getPrimaryInteger();
        switch (clashAnimationQueue.getCurrentActingPlayer())
        {
            case ActingPlayer.Primary:
                activeCard1 = player1.fighterInSlot(cardSlot);
                if (animationHelper.NotYetReachedDestination(ClashConstants.player1Deck, activeCard1))
                {
                    animationHelper.MoveTowardsPoint(ClashConstants.player1Deck, ClashConstants.getPlayer1CardLocation(cardSlot), ClashConstants.viewCardAnimationTime, activeCard1);
                }
                else
                {
                    player1.addToDeck(player1.removeFighterInSlot(cardSlot));
                    NextAnimation();
                }
                break;
            case ActingPlayer.Secondary:
                activeCard2 = player2.fighterInSlot(cardSlot);
                if (animationHelper.NotYetReachedDestination(ClashConstants.player2Deck, activeCard2))
                {
                    animationHelper.MoveTowardsPoint(ClashConstants.player2Deck, ClashConstants.getPlayer1CardLocation(cardSlot), ClashConstants.viewCardAnimationTime, activeCard2);
                }
                else
                {
                    player2.addToDeck(player2.removeFighterInSlot(cardSlot));
                    NextAnimation();
                }
                break;
        }
    ...
    void PlayFromDiscardAnimation()
    {
        switch (clashAnimationQueue.getCurrentActingPlayer())
        {
            case ActingPlayer.Primary:
                activeCard1 = player1.fighterFromDiscard(clashAnimationQueue.getCurrentAnimation().getPrimaryInteger());
                Vector3 fighter1Destination = ClashConstants.getPlayer1CardLocation(clashAnimationQueue.getCurrentAnimation().getSecondaryInteger());
                if (animationHelper.NotYetReachedDestination(fighter1Destination, activeCard1))
                {
                    animationHelper.MoveTowardsPoint(fighter1Destination, ClashConstants.player1Discard, ClashConstants.viewCardAnimationTime, activeCard1);
                } else
                {
                    player1.playFromDiscard(clashAnimationQueue.getCurrentAnimation().getSecondaryInteger(), clashAnimationQueue.getCurrentAnimation().getPrimaryInteger());
                    NextAnimation();
                }
                break;
            case ActingPlayer.Secondary:
                activeCard2 = player2.fighterFromDiscard(clashAnimationQueue.getCurrentAnimation().getPrimaryInteger());
                Vector3 fighter2Destination = ClashConstants.getPlayer2CardLocation(clashAnimationQueue.getCurrentAnimation().getSecondaryInteger());
                if (animationHelper.NotYetReachedDestination(fighter2Destination, activeCard2))
                {
                    animationHelper.MoveTowardsPoint(fighter2Destination, ClashConstants.player2Discard, ClashConstants.viewCardAnimationTime, activeCard2);
                }
                else
                {
                    player2.playFromDiscard(clashAnimationQueue.getCurrentAnimation().getSecondaryInteger(), clashAnimationQueue.getCurrentAnimation().getPrimaryInteger());
                    NextAnimation();
                }
                break;
        }
    ...
    void DestroyCardAnimation()
    {
        ...
        {
            case ActingPlayer.Primary:
                activeCard1 = player1.fighterInSlot(fighterNumber);
                ...
                else
                {
                    resetPower(activeCard1);
                    activeCard1.GetComponent<SortingGroup>().sortingLayerName = "Cards";
                    player1.destroyFighter(fighterNumber);
                    NextAnimation();
                }
                break;
            case ActingPlayer.Secondary:
                activeCard2 = player2.fighterInSlot(fighterNumber);
                ...
                else
                {
                    resetPower(activeCard2);
                    activeCard2.GetComponent<SortingGroup>().sortingLayerName = "Cards";
                    player2.destroyFighter(fighterNumber);
                    NextAnimation();
                }
                break;
        }
